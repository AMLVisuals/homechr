...
import type { DocUploadStatus } from '@/store/useStore';
...
export default function VenueForm({ initialData, onClose, onSuccess }: VenueFormProps) {
  ...
  const handleDocStatusChange = (docType: ComplianceDocType) => (status: DocUploadStatus) => {
    setPatronDocStatus(docType, status as DocUploadStatus);
  };
  ...
  return (
    <div className="flex flex-col h-full bg-[var(--bg-card)] text-[var(--text-primary)]">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
        ...
        {needsDocuments && (
          <div className="bg-[var(--bg-hover)] border-2 border-blue-500/30 rounded-2xl overflow-hidden">
            <button
            ...
            >
              ...
              {sections.documents ? <Eye className="w-4 h-4 text-[var(--text-secondary)]" /> : <EyeOff className="w-4 h-4 text-[var(--text-secondary)]" />}
            </button>

            {sections.documents && (
              <div className="p-4 border-t border-[var(--border)] space-y-2 animate-in fade-in slide-in-from-top-2">
                <div className="flex items-start gap-2 mb-4 p-3 rounded-xl bg-blue-500/10 border border-blue-500/20">
                  <AlertTriangle className="w-4 h-4 text-blue-400 mt-0.5 shrink-0" />
                  <p className="text-xs text-blue-300 leading-relaxed">
                    Ces documents sont nécessaires pour valider votre profil. Votre établissement sera visible une fois votre profil validé par notre équipe.
                  </p>
                </div>

                {PATRON_DOCUMENTS.map((doc) => {
                  const upload = patronDocUploads.find(u => u.type === doc.id);
                  return (
                    <DropZonePro
                      key={doc.id}
                      label={doc.label}
                      description={doc.description}
                      type={doc.id === 'IDENTITY' ? 'IDENTITY' : 'DOCUMENT'}
                      status={upload?.status || 'idle'}
                      onStatusChange={handleDocStatusChange(doc.id)}
                    />
                  );
                })}
              </div>
