  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h3 className="text-lg font-semibold text-white">Parcours Professionnel</h3>
          <p className="text-sm text-gray-400">Votre historique d'expérience</p>
        </div>
        {!isAdding && (
          <button
        ...
        {isAdding ? (
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <label className="text-sm text-gray-300">Intitulé du poste</label>
                <div className="relative">
                  <Briefcase className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-500" />
                  <input
                    {...register('role')}
                    className="w-full bg-black/50 border border-white/10 rounded-lg py-2.5 pl-10 pr-4 text-white focus:outline-none focus:border-blue-500 transition-colors"
                ...
                <div className="flex justify-between items-center mb-1">
                  ...
                  <label className="flex items-center gap-2 cursor-pointer group">
                    <input
                      ...
                      className="hidden"
                    />
                    <div className={`w-9 h-5 rounded-full relative transition-colors ${isCurrent ? 'bg-blue-600' : 'bg-zinc-700'}`}>
                    ...
                    <span className="text-xs text-gray-400 group-hover:text-gray-300 transition-colors">Poste actuel</span>
                  </label>
                </div>
                <Controller
                  control={control}
            ...
            <div className="space-y-2">
              ...
              <textarea
                ...
                rows={4}
                className="w-full bg-black/50 border border-white/10 rounded-lg py-2.5 px-4 text-white focus:outline-none focus:border-blue-500 transition-colors resize-none"
          ...
          <div className="relative pl-8 border-l-2 border-zinc-800 space-y-8">
            {sortedExperiences.map((exp, index) => (
              ...
              >
                {/* Timeline dot */}
                <div className={`absolute -left-[41px] top-1.5 w-5 h-5 rounded-full border-4 border-[#050505] ${!exp.endDate ? 'bg-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.5)]' : 'bg-zinc-700'}`} />

                <div className="bg-zinc-900/30 border border-white/5 rounded-xl p-5 hover:border-white/10 transition-colors">
