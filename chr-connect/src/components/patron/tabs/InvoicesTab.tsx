        ...
        <div className="pb-4">
          ...
          <div className="flex flex-col md:flex-row justify-between items-center md:items-center gap-4 mb-4 md:mb-8 p-4 md:p-0">
            <div className="text-center md:text-left w-full md:w-auto">
              <h2 className="text-3xl font-bold mb-1 bg-gradient-to-r from-[var(--gradient-heading-from)] via-[var(--gradient-heading-via)] to-[var(--gradient-heading-to)] bg-clip-text text-transparent">Factures</h2>
            ...
            <div className="hidden md:flex items-center gap-2 bg-[var(--bg-card)] p-1 rounded-xl border border-[var(--border)] w-full md:w-auto">
          ...
          <div className="grid grid-cols-3 gap-2 md:gap-4 mb-4 md:mb-8 px-4 md:px-0">
            <div
              ...
              className="bg-[var(--bg-card)] p-2 md:p-6 rounded-xl md:rounded-2xl border border-[var(--border)] flex flex-col justify-between cursor-pointer hover:bg-[var(--bg-hover)] active:scale-95 transition-all"
            >
              ...
              <div className="text-lg md:text-3xl font-bold text-[var(--text-primary)] mb-0 md:mb-1 text-center md:text-left truncate">2,820€</div>
              <div className="hidden md:block text-xs md:text-sm text-[var(--text-secondary)]">12 factures</div>
            </div>
            ...
            >
              <div className="flex flex-col md:flex-row justify-between items-center md:items-start mb-1 md:mb-4 gap-1 md:gap-0">
                ...
                <span className="text-[10px] md:text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider text-center md:text-left">Attente</span>
              </div>
              <div className="text-lg md:text-3xl font-bold text-[var(--text-primary)] mb-0 md:mb-1 text-center md:text-left truncate">890€</div>
              <div className="hidden md:block text-sm text-[var(--text-secondary)]">1 facture</div>
            </div>
            ...
            >
              <div className="flex flex-col md:flex-row justify-between items-center md:items-start mb-1 md:mb-4 gap-1 md:gap-0">
                ...
                <span className="text-[10px] md:text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider text-center md:text-left">Retard</span>
              </div>
              <div className="text-lg md:text-3xl font-bold text-[var(--text-primary)] mb-0 md:mb-1 text-center md:text-left truncate">230€</div>
              <div className="hidden md:block text-sm text-[var(--text-secondary)]">Action requise</div>
            </div>
          </div>
        </div>
        ...
        <div className="relative z-10 bg-[var(--bg-card)] rounded-t-3xl border-t border-[var(--border)] shadow-lg min-h-full md:z-auto md:bg-transparent md:rounded-none md:border-none md:shadow-none">
          ...
          <div className="sticky top-0 z-20 bg-[var(--bg-card)] pb-4 pt-4 px-4 md:px-0 rounded-t-3xl md:bg-transparent md:static md:z-auto md:rounded-none md:pt-0">
            <div className="px-4 md:px-0 pt-4 md:pt-0">
              {/* Mobile Search Bar */}
              <div className="md:hidden flex items-center gap-2 bg-[var(--bg-card)] p-1 rounded-xl border border-[var(--border)] w-full mb-4">
                <div className="relative flex-1">
                  <Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-[var(--text-muted)]" />
                  <input
                    type="text"
                ...
                {['ALL', 'PAID', 'PENDING', 'OVERDUE'].map((tab) => (
                  <button
                    ...
                    className={clsx(
                      ...
                      filter === tab ? "text-[var(--text-primary)]" : "text-[var(--text-muted)] hover:text-[var(--text-secondary)]"
                    )}
                  >
                    {tab === 'ALL' && 'Toutes'}
                    {tab === 'PAID' && 'Payées'}
            ...
            {filteredInvoices.map((invoice) => (
              ...
              >
                <div className="flex items-center gap-3 md:gap-4 flex-1 w-full md:w-auto">
                  <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl bg-[var(--bg-hover)] flex items-center justify-center shrink-0">
                    <FileText className="w-5 h-5 md:w-6 md:h-6 text-[var(--text-secondary)]" />
                  </div>
                  <div className="min-w-0 flex-1">
                    <h4 className="font-bold text-[var(--text-primary)] group-hover:text-blue-400 transition-colors text-sm md:text-base truncate">{invoice.serviceName}</h4>
                ...
                <div className="flex items-center gap-4 md:gap-6 w-full md:w-auto justify-between md:justify-end mt-1 md:mt-0 pl-[52px] md:pl-0">
                  <div className="text-left md:text-right">
                    <div className="font-bold text-[var(--text-primary)] text-base md:text-lg">{invoice.totalAmount.toFixed(2)}€</div>
                    <div className="text-[10px] md:text-xs text-[var(--text-muted)]">Échéance: {invoice.dueDate}</div>
                  </div>
