  ...
  return (
    <div className="fixed inset-0 z-[150] bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
      <motion.div 
        ...
        className="w-full max-w-4xl bg-[#121212] border border-white/10 rounded-3xl overflow-hidden flex flex-col max-h-[90vh] shadow-2xl"
      >
        ...
        <div className="relative shrink-0">
          ...
          <div className="px-8 pb-6 -mt-12 flex flex-col md:flex-row gap-6 items-start md:items-end">
            <div className="relative">
              <div className="w-24 h-24 md:w-32 md:h-32 rounded-full border-4 border-[#121212] overflow-hidden bg-gray-800 shadow-xl">
                <img src={provider.avatarUrl} alt={provider.firstName} className="w-full h-full object-cover" />
              </div>
              {provider.badges.includes('VERIFIED') && (
                <div className="absolute bottom-2 right-2 bg-blue-500 text-white p-1 rounded-full border-2 border-[#121212]" title="Profil Vérifié">
            ...
            <div className="flex-1">
              <div className="flex flex-wrap items-center gap-3 mb-1">
                <h2 className="text-2xl md:text-3xl font-bold text-white">{provider.firstName} {provider.lastName}</h2>
              ...
              <div className="flex flex-wrap gap-4 text-sm text-gray-400">
                ...
                <div className="flex items-center gap-1 text-yellow-500">
                  ...
                  <span className="font-bold text-white">{provider.stats.rating}</span> 
                  <span className="text-gray-500">({provider.reviews.length} avis)</span>
                </div>
                <div className="flex items-center gap-1">
            ...
            <div className="flex flex-col gap-2 w-full md:w-auto">
              <div className="flex items-center gap-2 text-sm text-gray-400 mb-1">
                ...
                Dispo: <span className="text-white font-medium">{provider.availability.nextSlot}</span>
              </div>
              <button 
                onClick={onBook}
          ...
          <AnimatePresence mode='wait'>
            {activeTab === 'ABOUT' && (
              ...
              >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {featuredWork.map((work) => (
                        <div key={work.id} className="space-y-2">
                          ...
                          <p className="text-sm text-gray-400 font-medium text-center">{work.title}</p>
                        </div>
                      ))}
                    </div>
                  </section>
                ...
                <section>
                  <h3 className="text-lg font-bold text-white mb-4">Langues</h3>
                  <div className="flex gap-2">
                    {provider.languages.map(lang => (
                      <span key={lang} className="px-3 py-1 bg-white/5 rounded-lg text-sm text-gray-300 border border-white/10">
                        {lang}
                      </span>
                    ))}
                  </div>
                ...
                <section>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="bg-white/5 p-4 rounded-2xl border border-white/10 text-center">
                      ...
                      <div className="text-xs text-gray-500 mt-1">Réponse</div>
                    </div>
                    <div className="bg-white/5 p-4 rounded-2xl border border-white/10 text-center">
                    ...
                    <div className="bg-white/5 p-4 rounded-2xl border border-white/10 text-center">
                      ...
                      <div className="text-xs text-gray-500 mt-1">Missions</div>
                    </div>
                    <div className="bg-white/5 p-4 rounded-2xl border border-white/10 text-center">
            ...
            {activeTab === 'SKILLS' && (
              ...
              >
                <section>
                  <h3 className="text-lg font-bold text-white mb-4">Compétences Techniques</h3>
                ...
                <section>
                  <h3 className="text-lg font-bold text-white mb-4">Certifications & Documents</h3>
                  <div className="space-y-3">
                    {provider.certifications.map(cert => (
                      <div key={cert.id} className="flex items-center justify-between p-4 bg-white/5 rounded-xl border border-white/10">
                        <div className="flex items-center gap-3">
                          <Award className="w-8 h-8 text-yellow-500" />
                          <div>
                            <div className="font-bold text-white">{cert.name}</div>
                            <div className="text-sm text-gray-400">{cert.issuer} • {cert.dateObtained}</div>
                          </div>
                        </div>
                        {cert.isVerified && (
                    ...
                    {provider.certifications.length === 0 && (
                      <p className="text-gray-500 italic">Aucune certification affichée.</p>
                    )}
                  </div>
                </section>
                  ...
                  <div className="relative border-l-2 border-white/10 ml-3 space-y-8 py-2">
                    {provider.experiences.map((exp, idx) => (
                      <div key={exp.id} className="relative pl-8">
                        <div className="absolute -left-[9px] top-0 w-4 h-4 rounded-full bg-[#121212] border-2 border-blue-500" />
                        <div>
                    ...
                    {provider.experiences.length === 0 && (
                      <p className="text-gray-500 italic pl-8">Aucune expérience renseignée.</p>
                    )}
                  </div>
                </section>
              </motion.div>
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {provider.portfolio.map(item => (
                    <div key={item.id} className="group relative aspect-video bg-gray-800 rounded-xl overflow-hidden border border-white/10">
                      {item.type === 'BEFORE_AFTER' ? (
                         <div className="w-full h-full pointer-events-none">
                            ...
                            />
                            <div className="absolute top-2 right-2 bg-black/60 backdrop-blur-md p-1.5 rounded-lg z-10 border border-white/10">
            ...
            {activeTab === 'REVIEWS' && (
              ...
              >
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-lg font-bold text-white">Avis clients ({localReviews.length})</h3>
                  <button
